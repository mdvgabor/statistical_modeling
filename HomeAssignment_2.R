...
#helper_table$prop <- helper_table$sample_size / sum(helper_table$sample_size)
#helper_table$n_h <- n * helper_table$prop
#mse_ps_theoretical <- sum((helper_table$prop^2) * (helper_table$sd^2 / helper_table$n_h))
# * correction_factor
...
# we have to decide which one will we use: Laci's version or ChatGPT's version
